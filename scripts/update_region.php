<?php 
	...
	while(! feof($file)){
		...
		if($data['0'] != 'id'){
			...
			$telephone 		= $data['12'];
			$companyName 	= $data['4'];
			$bill_address1  = $data['7'];
			$bill_address2  = $data['8'];

			$ship_address1	= $data['19'];
			$ship_address2	= $data['20'];
			...
			$entity_id  = $read->fetchOne($sql); //fetchRow($sql), fetchOne($sql),...

			if($entity_id != ''){
				
				$sql = "SELECT * FROM customer_address_entity AS cae INNER JOIN customer_entity_int AS cei ON 'cei.entity_id'= 'cae.parent_id' WHERE 'cae.parent_id'='".$entity_id."' AND cei.attribute_id = 13";

				$arr = $read->fetchRow($sql);
			...
			// if(1){
				...
				Mage::log($email,null,'custom_added_customer.log');

			}
			else{
				Mage::log($email,null,'custom_existsCustomer.log');				
			}
		}
	}
