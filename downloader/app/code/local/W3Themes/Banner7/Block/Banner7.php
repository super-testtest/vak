...
-------------------------------------------------------------------------*/ 
class W3Themes_Banner7_Block_Banner7 extends Mage_Core_Block_Template
{
	...
	public function getDataBanner7()
    {
		...
		$select = $read->select()
		   ->from($slideTable,array('banner7_id','title','link','rel','description','image','order', 'store_id','status'))
		   ->where('status=?',1);
