...
IWD.OPC = {
		...
		checkCOD: function()
		{
			if (jQuery('input[name="business_type"]:checked').val() == 0)
	        {
	        	jQuery('#p_method_cashondelivery').removeAttr('checked');
	            jQuery('input[name="billing[business_type]"]').val(0);
	            jQuery(".businessType").hide();
	            jQuery("#p_method_cashondelivery").hide();
	            jQuery("label[for=p_method_cashondelivery]").hide();
	        }
	        else if(jQuery('input[name="business_type"]:checked').val() == 1)
	        {
	            jQuery('input[name="billing[business_type]"]').val(1);
	            jQuery(".businessType").show();
...
IWD.OPC.Billing = {
		...
		initChangeAddress: function(){
			...
			$j('input[name="billing[telephone]"]').keyup(function(){
			...
			$j('#opc-address-form-billing input').focus(function(){
				IWD.OPC.Billing.bill_need_update = true;
				if(!(lastUsed == "billing[telephone]" && this.name == "billing[fax]"))
				{
					setTimeout(function(){
						clearTimeout(IWD.OPC.Checkout.ajaxProgress);
					}, 150);
