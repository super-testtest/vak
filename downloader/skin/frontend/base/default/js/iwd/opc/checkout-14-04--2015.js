...
IWD.OPC.Billing = {
		...
		initChangeAddress: function(){
			...
			/*Start : Client requested changes : 13-04-2015*/
			$j('input[name="billing[fax]"]').mouseleave(function(){
					alert("yes mouseleave");
					IWD.OPC.Billing.validateForm();
			});
			$j('input[name="billing[fax]"]').focusout(function(){
				//var valid = IWD.OPC.Billing.validateAddressForm();
				//if (valid){

					if(typeof(focusoutFlag) == "undefined")
					{
						window.focusoutFlag = 1;
					}
					if(typeof(focusoutFlag) != "undefined" && focusoutFlag == 1)
					{
						alert(focusoutFlag);
						focusoutFlag++;
						IWD.OPC.Billing.validateForm();
			...
			/*End : Client requested changes : 13-04-2015*/
		...
		validateAddressForm: function(form){
			  ...
			  var addressForm = new Validation('opc-address-form-billing', { onSubmit : false, stopOnFirst : true, focusOnError : false});
...
IWD.OPC.Shipping = {
		...
		validateAddressForm: function(form){
			  ...
			  var addressForm = new Validation('opc-address-form-shipping', { onSubmit : false, stopOnFirst : true, focusOnError : false});
			  if (addressForm.validate()){				  		 
				  return true;
			  }else{				 
				  return false;
