...
IWD.OPC = {
		...
		initSaveOrder: function(){
			...
			//if(IWD.OPC.btnClick > IWD.OPC.btnCheck){
				// IWD.OPC.btnCheck++;
...
IWD.OPC.Billing = {
		...
		validateAddressForm: function(form){
			...
			var addressForm = new Validation('opc-address-form-billing', { onSubmit : false, stopOnFirst : false, focusOnError : true});
...
IWD.OPC.Coupon = {
		...
		prepareResponse: function(response){
			...
			IWD.OPC.Checkout.xhr = null;
			IWD.OPC.Checkout.hideLoader();
			$j_opc('#discount-giftcard-form').remove();
