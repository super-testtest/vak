...
function groupBBFix(selected) {
	...
	var ref = $j.data(selected, 'ref');
	var matched = null;
	
	if(ref) {
		var copy = $j(ref).children().clone().attr('visibility', 'hidden');
		$j(svgroot_).append(copy);
		matched = copy.filter('line, path');
	} else {
		matched = $j(selected).find('line, path');
	}
	...
	if(matched.length) {
		...
		if(issue) {
			var elems = ref ? copy : $j(selected).children();
	...
	default:
		...
		if(elname === 'use') {
			...
			try { ret = selected.getBBox();} 
				...
				var fo = $j(selected).closest("foreignObject");
				if(fo.length) {
					try {
						ret = fo[0].getBBox();
					} catch(e) {
...
if (svgedit.browser.supportsSelectors()) {
	...
	svgedit.utilities.getElem = function(id) {
		...
		return $j(svgroot_).find('[id=' + id + ']')[0];
	};
}
