...
// 									key: btn.key,
			...
			var getPaint = function(color, opac, type) {
				...
				if (color.indexOf("url(#") === 0) {
					...
					$j(".prb_text_colour").change(function(e) 
					{
					    var tmp = $j("select[name=prb_text_colour]").val();
    					var hexcode = colourNameToHex(tmp);
						$j("#prb_nametag_colour").val(hexcode); 
						opts = {
							alpha: opac,
							solidColor: hexcode
						};
...
// 				var opacity = (picker == 'stroke' ? $j('#stroke_opacity') : $j('#fill_opacity'));
			...
			var PaintBox = function(container, type) {
				...
				var svgdocbox = new DOMParser().parseFromString(
					//'<svg xmlns:xlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg"><rect width="25" height="25" fill="#' + cur.color + '" opacity="' + cur.opacity + '"/>	<image xlink:href="http://192.168.0.222/dnb_products/allinone_html5product/js/html5/jgraduate/images/picker.gif"/><defs><linearGradient id="gradbox_"/></defs></svg>', 'text/xml');
		...
		// 	}
			...
			//$j('#rect_rx').SpinButton({ min: 0, max: 1000, step: 1, callback: changeRectRadius });
			//$j('#stroke_width').SpinButton({ min: 0, max: 99, step: 1, smallStep: 0.1, callback: changeStrokeWidth });
			//$j('#angle').SpinButton({ min: -180, max: 180, step: 5, callback: changeRotationAngle });
			//$j('#font_size').SpinButton({ step: 1, min: 0.001, stepfunc: stepFontSize, callback: changeFontSize });
			//$j('#group_opacity').SpinButton({ step: 5, min: 0, max: 100, callback: changeOpacity });
			//$j('#blur').SpinButton({ step: .1, min: 0, max: 10, callback: changeBlur });
			$j('#zoom').SpinButton({ min: 0.001, max: 10000, step: 50, stepfunc: stepZoom, callback: changeZoom })
				// Set default zoom
				.val(svgCanvas.getZoom() * 100);

			/*$j("#workarea").contextMenu({
					menu: 'cmenu_canvas',
					inSpeed: 0
				},
