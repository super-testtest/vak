...
(function($j, version)
{
  ...
  var Slider = // encapsulate slider functionality for the ColorMap and ColorBar - could be useful to use a jQuery UI draggable for this with certain extensions
      function(bar, options)
      {
        var $this = this, // private properties, methods, and events - keep these variables and classes invisible to outside code
          ...
          mouseDown = // bind the mousedown to the bar not the arrow for quick snapping to the clicked location
            function(e)
            {
              ...
              $j(document).bind('mousemove', mouseMove).bind('mouseup', mouseUp);
          ...
          mouseUp = // unbind the document events - they aren't needed when not dragging
            function(e)
            {
              $j(document).unbind('mouseup', mouseUp).unbind('mousemove', mouseMove);
              e.stopPropagation();
              e.preventDefault();
              return false;
            },
          ...
          bind =
            function (callback)
            {
              if ($j.isFunction(callback)) changeEvents.push(callback);
            },
          unbind =
            function (callback)
            {
              if (!$j.isFunction(callback)) return;
              var i;
              while ((i = $j.inArray(callback, changeEvents)) != -1) changeEvents.splice(i, 1);
            },
          destroy =
            function()
            {
        ...
        $j.extend(true, $this, // public properties, methods, and event bindings - these we need to access from other controls
          {
            val: val,
            range: range,
            bind: bind,
    ...
    ColorValuePicker = // controls for all the input elements for the typing in color values
      function(picker, color, bindedHex, alphaPrecision)
      {
        ...
        $j.extend(true, $this, // public properties and methods
          {
            destroy: destroy
          });
  ...
  var Color = $j.jPicker.Color, List = $j.jPicker.List, ColorMethods = $j.jPicker.ColorMethods; // local copies for YUI compressor
  $j.fn.jPicker =
    function(options)
    {
      ...
      return this.each(
        function()
        {
          var $this = this, settings = $j.extend(true, {}, $j.fn.jPicker.defaults, options); // local copies for YUI compressor
          if ($j($this).get(0).nodeName.toLowerCase() == 'input') // Add color picker icon if binding to an input element and bind the events to the input
          {
            ...
            else if (ColorMethods.validateHex($j($this).val()))
            {
              settings.color.active = new Color({ hex: $j($this).val(), a: settings.color.active.val('a') });
              settings.color.current = new Color({ hex: $j($this).val(), a: settings.color.active.val('a') });
            }
          }
          if (settings.window.expandable)
            $j($this).after('<span class="jPicker"><span class="Icon"><span class="Color">&nbsp;</span><span class="Alpha">&nbsp;</span><span class="Image" title="Click To Open Color Picker">&nbsp;</span><span class="Container">&nbsp;</span></span></span>');
          ...
          var isLessThanIE7 = parseFloat(navigator.appVersion.split('MSIE')[1]) < 7 && document.body.filters, // needed to run the AlphaImageLoader function for IE6
            ...
            cancelClicked =
              function()
              {
                ...
                $j.isFunction(cancelCallback) && cancelCallback.call($this, color.active, cancelButton);
              },
            okClicked =
              function()
              {
                ...
                settings.window.expandable && hide.call($this);
                $j.isFunction(commitCallback) && commitCallback.call($this, color.active, okButton);
              },
            iconImageClicked =
              function()
              {
            ...
            moveBarMouseDown =
              function(e)
              {
                ...
                $j(document).bind('mousemove', documentMouseMove).bind('mouseup', documentMouseUp);
            ...
            documentMouseMove =
              function(e)
              {
                ...
                if (settings.window.expandable && !$j.support.boxModel) container.prev().css({ left: container.css("left"), top: container.css("top") });
                e.stopPropagation();
                e.preventDefault();
                return false;
              },
            documentMouseUp =
              function(e)
              {
                $j(document).unbind('mousemove', documentMouseMove).unbind('mouseup', documentMouseUp);
                e.stopPropagation();
                e.preventDefault();
                return false;
              },
            quickPickClicked =
              function(e)
              {
                e.preventDefault();
                e.stopPropagation();
                color.active.val('ahex', $j(this).attr('title') || null, e.target);
                return false;
              },
            commitCallback = $j.isFunction($arguments[1]) && $arguments[1] || null,
            liveCallback = $j.isFunction($arguments[2]) && $arguments[2] || null,
            cancelCallback = $j.isFunction($arguments[3]) && $arguments[3] || null,
            show =
              function()
              {
                color.current.val('ahex', color.active.val('ahex'));
                var attachIFrame = function()
                  {
                    if (!settings.window.expandable || $j.support.boxModel) return;
            ...
            initialize =
              function()
              {
                var win = settings.window,
                    popup = win.expandable ? $j($this).next().find('.Container:first') : null;
                container = win.expandable ? $j('<div/>') : $j($this);
                ...
                if (win.expandable)
                {
                  ...
                  if($j(document.body).children('div.jPicker.Container').length==0)$j(document.body).prepend(container);
                  else $j(document.body).children('div.jPicker.Container:last').after(container);
                  container.mousedown(
                    function()
                    {
                      $j(document.body).children('div.jPicker.Container').css({zIndex:10});
                  ...
                  container.css( // positions must be set and display set to absolute before source code injection or IE will size the container to fit the window
                    {
                      left:
                        ...
                        win.position.x == 'screenCenter' ? (($j(document).width() >> 1) - 260) + 'px' : (popup.offset().left + parseInt(win.position.x)) + 'px',
                      position: 'absolute',
                ...
                $j.isFunction(liveCallback) && color.active.bind(liveCallback);
          ...
          $j.extend(true, $this, // public properties, methods, and callbacks
            {
  ...
  $j.fn.jPicker.defaults = /* jPicker defaults - you can change anything in this section (such as the clientPath to your images) without fear of breaking the program */
      {
      window:
        {
      ...
      color:
        {
          ...
          active: new Color({ ahex: '#ffcc00ff' }), /* acceptable values are any declared $j.jPicker.Color object or string HEX value (e.g. #ffc000) WITH OR WITHOUT the "#" prefix */
          quickList: /* the quick pick color list */
            [
              new Color({ h: 360, s: 33, v: 100 }), /* acceptable values are any declared $j.jPicker.Color object or string HEX value (e.g. #ffc000) WITH OR WITHOUT the "#" prefix */
