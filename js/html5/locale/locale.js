...
var svgEditor = (function($j, Editor) {
	...
	Editor.readLang = function(langData) {
		...
		$j.each(more, function(i, m) {
			if(m.data) {
				langData = $j.merge(langData, m.data);
			}
		});
		...
		// Context menus
		var opts = {};
		$j.each(['cut','copy','paste', 'paste_in_place', 'delete', 'group', 'ungroup', 'move_front', 'move_up', 'move_down', 'move_back'], function() {
			opts['#cmenu_canvas a[href="#' + this + '"]'] = tools[this];
		});

		$j.each(['dupe','merge_down', 'merge_all'], function() {
	...
	Editor.putLocale = function(given_param, good_langs){
		...
		if(given_param) {
			...
			if($j.inArray(lang_param, good_langs) == -1 && lang_param !== 'test') {
				lang_param = "en";
			}
