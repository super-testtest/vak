	...
	this.selectorRect = this.selectorGroup.appendChild(
		svgFactory_.createSVGElement({
			...
			'attr': {
				...
				'fill': 'none',
				'stroke': '#3399FF', // stroke color changed by RWS
				'stroke-width': '1',
...
svgedit.select.Selector.prototype.resize = function() {
	var selectedBox = this.selectorRect,
		...
		//current_zoom = 0.5; //svgFactory_.currentZoom();
	...
	if(tagName === 'g' && !$j.data(selected, 'gsvg')) {
...
svgedit.select.SelectorManager.prototype.initGroup = function() {
	...
	for (var dir in this.selectorGrips) {
		var grip = svgFactory_.createSVGElement({
			...
			'attr': {
				...
				'fill': '#56B9E9',	//fill color changed by RWS
				'r': gripRadius,
	...
	this.rotateGripConnector = this.selectorGripsGroup.appendChild(
		svgFactory_.createSVGElement({
			...
			'attr': {
				...
				'stroke': '#56B9E9',	//Stroke color changed by RWS
				'stroke-width': '1'
			}
		})
	);
	...
	$j.data(this.rotateGrip, 'type', 'rotate');

	if($j('#canvasBackground').length) return;
	...
	var rect = svgFactory_.createSVGElement({
		...
		'attr': {
			...
			'stroke-width': 1,
			'stroke': '#000',
			//Edited : Ashesh 24-2-2014 as in iphone 4 its was showing white color
			//'fill': '#FFF',
			'fill': 'transparent',
			//Edited : End
			'style': 'pointer-events:none'
