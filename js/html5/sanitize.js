...
var nsRevMap_ = {};
$j.each(nsMap_, function(key,value){
	nsRevMap_[value] = key;
});
...
var svgWhiteListNS_ = {};
$j.each(svgWhiteList_, function(elt,atts){
	var attNS = {};
	$j.each(atts, function(i, att){
		if (att.indexOf(':') >= 0) {
			var v = att.split(':');
...
svgedit.sanitize.sanitizeSvg = function(node) {
	...
	if (allowedAttrs != undefined) {
		...
		$j.each(se_attrs, function(i, attr) {
		...
		$j.each(["clip-path", "fill", "filter", "marker-end", "marker-mid", "marker-start", "mask", "stroke"],function(i,attr) {
