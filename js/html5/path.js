...
svgedit.path.init = function(editorContext) {
	...
	$j.each(pathFuncsStrs, function(i,s) {
...
svgedit.path.addPointGrip = function(index, x, y) {
	...
	if (!pointGrip) {
		...
		var grip = $j('#pathpointgrip_'+index);
...
svgedit.path.Segment.prototype.selectCtrls = function(y) {
	$j('#ctrlpointgrip_' + this.index + 'c1, #ctrlpointgrip_' + this.index + 'c2').
		attr('fill', y ? '#0FF' : '#EEE');
};
...
svgedit.path.Path.prototype.init = function() {
	// Hide all grips, etc
	$j(svgedit.path.getGripContainer()).find("*").attr("display", "none");
