...
svgEditor.addExtension("shapes", function() {
	...
	function loadLibrary(cat_id) {
		...
		if(!lib) {
			$j('#shape_buttons').html('Loading...');
			$j.getJSON(jspath+'extensions/shapelib/' + cat_id + '.json', function(result, textStatus) {
	...
	return {
		svgicons: jspath+"extensions/ext-shapes.xml",
		buttons: [{
			id: "tool_shapelib",
