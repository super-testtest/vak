...
class Package {
   ...
   * @param array $files
   */
  function __construct($uploadSession, $index, $saveIntoCache = false, $initialLoad = false) {
    ...
    $requestCount = (!empty($post[PostFields::requestCount])) ? intval($post[PostFields::requestCount]) : -1;
    $cache = $this->_uploadSession->getUploadCache();

    if ($saveIntoCache || $requestCount != 1) {
      $cache->saveRequestData($uploadSessionId, $this->_packageIndex, $post, $files);
      $this->_cached = true;

      $this->_completed = $cache->isPackageCompleted($uploadSessionId, $this->_packageIndex, $requestCount, $uploadSession->completedPackagesCount);
    } else {
      $this->_completed = true;
