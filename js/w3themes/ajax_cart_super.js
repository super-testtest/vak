            ...
            $jq(this).live('click',function(){
                ...
                if(linkToCart.search('checkout/cart/add')!= -1 || linkToCart.search('ajaxcartsuper/ajaxcart/add') !=-1) {
                    ...
                    try {
                        ...
                        else if(parent.find('.price').attr('id')) {
                            
                            if(parent.find('.price').attr('id').search('old-price-')!= -1){
                                productId = parent.find('.price').attr('id').replace('old-price-','');
                            }

                            if(parent.find('.price').attr('id').search('price-including-tax-')!= -1){
                                 productId = parent.find('.price').attr('id').replace('price-including-tax-','');
                            }

                            if(parent.find('.price').attr('id').search('price-excluding-tax-')!= -1){
                                productId = parent.find('.price').attr('id').replace('price-excluding-tax-','');
                            }
                        }
                        else {
...
function showBoxInfo(product_info) {
    ...
    str += "<div><a href='"+cart_url+"' target='_parent'  id ='shopping_cart'>"+title_shopping_cart+"</a></div></div>";
...
function showBoxInfoCompare(product_info) {
    ...
    str += "<div id ='product_info_box'>"+product_info+"</div>";
    str += "<div><a href= 'javascript:void(0)'  id ='continue_shopping'>"+title_shopping_cart+"</a></div>";
    str += "<div><a id='shopping_cart'  href='"+cart_url+"'>"+title_shopping_compare+"</a></div></div>";
    $jq('.loadding_ajaxcart').html(str);
    $jq(str).insertAfter('#wraper_ajax');
    $jq('#wraper_ajax').css('opacity',0.8);
}
        ...
        $jq.ajax({
            ...
            success: function(data){
                if(data.status==1) {
                    ...
                    if (data.status==100)
                    {
                        if(data.type_product_custom==1){
                             hideLoadingAnimation();
                             location.href = data.type_product_custom_value;
                             return false;
                        }                        
                    }
                }
