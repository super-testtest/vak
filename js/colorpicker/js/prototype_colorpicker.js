var colorPicker=Class.create({
    ...
    this.cssPositionFixed = (this.cp.getStyle('position') == 'fixed');
	...
	change:function(el){
		...
		if(this.options.updateOnChange){
			if(this.el.nodeName == 'INPUT') 
			{
				...
				{
					count = 0;
					pos = this.el.value.indexOf("|");
					while ( pos != -1 ) {
					   count++;
					   pos = this.el.value.indexOf( "|",pos + 1 );
					}

					if (count == 0)
					{
						alert('Please fill both the labels seprated by "|" like Colorname | Printer Colorname' );
					}
					if (count > 0)
					{
						if (count == 1)
						{
							this.el.value = this.el.value + ' | #'+this.HSBToHex(col);
						}

						if (count == 2)
						{
							this.el.value = this.el.value.replace(this.el.value.slice(-6), this.HSBToHex(col), "gi");
						}

					}
				}
				else
					alert('Please add the labels for the color to show in the frontend and for the printer');
			}
	...
	moveIncrement:function(ev){
		this.current.field.value = Math.max(0, Math.min(this.current.max, parseInt(this.current.val + this.pointerY(ev) - this.current.y, 10)));
	...
	moveHue:function(ev){
		this.fields[4].value = parseInt(360*(150 - Math.max(0,Math.min(150,(this.pointerY(ev) - this.current.y))))/150, 10)
	...
	moveSelector:function(ev){
		this.fields[6].value = parseInt(100*(150 - Math.max(0,Math.min(150,(this.pointerY(ev) - this.current.pos.top))))/150, 10);
		this.fields[5].value = parseInt(100*(Math.max(0,Math.min(150,(this.pointerX(ev) - this.current.pos.left))))/150, 10);
		this.change(ev.element());
		return false;
	},
	...
	pointerY: function(ev) {
  	return (this.cssPositionFixed)? ev.pointerY()-document.viewport.getScrollOffsets().top : ev.pointerY();
	},
	pointerX: function(ev) {
  	return (this.cssPositionFixed)? ev.pointerX()-document.viewport.getScrollOffsets().left : ev.pointerX();	
	}
