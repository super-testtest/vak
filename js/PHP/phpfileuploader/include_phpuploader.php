...
class PhpUploader
{
	...
	function Render($mysession_id)
	{
		try
		{
			echo $this->GetString($mysession_id);
		}
		catch(Exception $x)
		{
	...
	function GetString($mysession_id)
	{
		$this->Maintain();
		
		session_id($mysession_id);
		/*session_save_path($SesSavePath);*/
		session_start();
		...
		/*$code.=$this->_GenerateAttribute("SesSavePath");*/
		...
		//TODO:Opera!!
		$code.=" onload='this.style.display=&quot;none&quot; ; ".PhpUploader_GetNamespace()."_AjaxUploader_Initialize(this.id);' onerror='this.onload()' /*SesSavePath='" .$SesSavePath. "'*/ ContextValue='" .$mysession_id. "' />";
		return $code;
	}
	...
	{		
		...
		/*$this->_SaveSecuritySetting("SesSavePath");*/
	...
	{		
		...
		/*session_save_path('/var/www/html/vaktrommet/var/session');*/
		session_start();

		$this->_LaveSecuritySetting("_SourceFileName");
		$this->_LaveSecuritySetting("MaxSizeKB");
		/*$this->_LaveSecuritySetting("SesSavePath");*/
	...
	{	
		...
		{
			throw (new Exception("$this->Name : 1 Session expired! Please refresh the page and try again."));
		}
	...
	function WriteValidationOK($message,$newFileName=null)
	{
		if(!$message)$message="";
		...
		//Parth Added : 17-06-14 Used window.top.document 
		echo("<script type='text/javascript'>");
		echo("if(window.parent.CurrentUpload){
				window.parent.CurrentUpload.UploadOK('" . $uploadid . "','" . $this->_fileguid . "','" . PhpUploader_JSEncode($message) . "');
				
				window.parent.document.getElementById('".$logoFileName."').value = '".$newFileName."';
			}");
		echo("</script>");
