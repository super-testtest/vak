...
class Mage_Paypal_Model_Ipn
{
    ...
    public function processIpnRequest(array $request, Zend_Http_Client_Adapter_Interface $httpAdapter = null)
    {
        ...
        Mage::log($this->_request,null,'IPN.log');
        try {
            if (isset($this->_request['txn_type']) && 'recurring_payment' == $this->_request['txn_type']) {
                Mage::log('106',null,'IPN.log');
                $this->_getRecurringProfile();
                Mage::log('108',null,'IPN.log');
                if ($httpAdapter) {
                    Mage::log('110',null,'IPN.log');
                    $this->_postBack($httpAdapter);
                    Mage::log('112',null,'IPN.log');
                }
                $this->_processRecurringProfile();
                Mage::log('115',null,'IPN.log');
            } else {
                Mage::log('117',null,'IPN.log');
                $this->_getOrder();
                Mage::log('119',null,'IPN.log');
                if ($httpAdapter) {
                    Mage::log('121',null,'IPN.log');
                    $this->_postBack($httpAdapter);
                    Mage::log('123',null,'IPN.log');
                }
                $this->_processOrder();
                Mage::log('126',null,'IPN.log');
            }
            Mage::log('Payapal order IPN csllled',null,'ppppp.log');
        } catch (Exception $e) {
            Mage::log($e,null,'IPN.log');
    ...
    protected function _postBack(Zend_Http_Client_Adapter_Interface $httpAdapter)
    {
            ...
            Mage::log($response,null,'IPN.log');
            if($response == 'VERIFIED'){
                if(isset($this->_request['invoice']) && $this->_request['invoice'] != ''){
                    $orderId = $this->_request['invoice'];
