                        ...
                        if ($baseBalance > 0) {
                            ...
                            foreach ($address->getAllItems() as $item) {
                                ...
                                if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                                    ...
                                    // Zend_debug::dump($baseDiscountTotal);die();
                                }
                            }
...
//
//                            if ($baseDiscount > 0) {
//                                if ($itemDiscount < $baseDiscount) {
//                                    $discountGiftcardCodes = $itemDiscount;
//                                    $baseDiscount -= $itemDiscount;
//                                } else {
//                                    $discountGiftcardCodes = $baseDiscount;
//                                    $baseDiscount = 0;
//                                }
//                            }
//
//                            $child->setMagestoreBaseDiscount($child->getMagestoreBaseDiscount() + $discountGiftcardCodes);
//                            $child->setBaseGiftVoucherDiscount($child->getBaseGiftVoucherDiscount() + $discountGiftcardCodes);
//                            $child->setGiftVoucherDiscount($child->getGiftVoucherDiscount() + $store->convertPrice($discountGiftcardCodes));
//                        } else {
                            if (Mage::helper('tax')->priceIncludesTax())
                                $itemDiscount = $child->getRowTotalInclTax() - $child->getMagestoreBaseDiscount() - $child->getDiscountAmount();
                            else
