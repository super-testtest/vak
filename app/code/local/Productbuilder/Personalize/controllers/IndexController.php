...
class Productbuilder_Personalize_IndexController extends Mage_Core_Controller_Front_Action//Varien_Object
{
    ...
    public function createCustomImageAction()
    {
        ...
        $labelPath = '/media/catalog/product/productbuilder/label_img_'.$var.'.png';
        $labelname = getcwd().$labelPath;

        $flabel = fopen($labelname, 'w') or die("can't open file");
        $labelimage = explode('base64,',$post_data['labelImg']);
        //Mage::log($labelimage);
        //$m=$post_data['']
        fwrite($flabel, base64_decode($labelimage[1]));
        fclose($flabel);

        $fileSize = fileSize('/var/www/html/vaktrommet_dev'.$labelPath);
        if($fileSize == 0){
            Mage::log('======== Zero Size Image Create ===========',null,'labelImage.log');
            Mage::log('$fileSize : '.$fileSize,null,'labelImage.log');
            Mage::log('$post_data[labelImg] : '.$post_data['labelImg'],null,'labelImage.log');
            Mage::log('base64_decode($labelimage[1]) : '.base64_decode($labelimage[1]),null,'labelImage.log');

        }


       // Mage::log($labelimage[1]);
        // End :: Create Lable image
        ...
        if($post_data['cartEditMode'] && $post_data['cartEditMode'] == 1)
        {
         ...
         /*$svglabel = $_POST['svg_1'];
        Mage::log($filePath);   */
        //$m=$post_data['product_highres_svg'];
  ...
  //;
       ...
       // Mage::dispatchEvent('svghighressoutiondivcode');
 }
    ...
    public function getImageResolutionAction(){
        $post_data = $this->getRequest()->getParams();
        $url = $post_data['url'];
