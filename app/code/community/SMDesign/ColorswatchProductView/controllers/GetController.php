...
class SMDesign_ColorswatchProductView_GetController extends Mage_Core_Controller_Front_Action {
	...
	function mainImageAction() {
		...
		if (Mage::getStoreConfig('smdesign_smdzoom/zoom/enabled') && $_product->getData('enable_zoom_plugin') == 1) {
			...
			$bigImageWidth 	= 600;
	 		$bigImageHeight = 600;
	 		$mainImageWidth = 360;//$this->getRequest()->getParam('img_width', null);
	 		$mainImageHeight= 360;//$this->getRequest()->getParam('img_height', null);
	 		$thumbImageWidth = 100;
...
$$('.cloud-zoom').first().href = '<?php echo $images[0]['big_image']?>?rand=' + Math.random();
$$('.cloud-zoom img').first().src = '<?php echo $images[0]['image']?>?rand=' + Math.random();
<?php endif; ?>
...
<?php foreach ($images as $key=>$image) : ?>
	...
	<?php else : ?>	
		$(li).update("<a href=\"<?php echo  $image['big_image']; ?>\" class=\"cloud-zoom-gallery\" rel=\"useZoom: 'zoom1', smallImage: '<?php echo  $image['image']; ?>'\"><img src=\"<?php echo  $image['thumb'] ?>\" alt=\"<?php echo  $image['label'] ?>\" /></a>");
	<?php endif; ?>
