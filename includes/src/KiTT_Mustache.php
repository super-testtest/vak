...
class KiTT_Mustache {
	...
	protected $_throwsExceptions = array(
		KiTT_Mustache_Exception::UNKNOWN_VARIABLE         => false,
		KiTT_Mustache_Exception::UNCLOSED_SECTION         => true,
		KiTT_Mustache_Exception::UNEXPECTED_CLOSE_SECTION => true,
		KiTT_Mustache_Exception::UNKNOWN_PARTIAL          => false,
		KiTT_Mustache_Exception::UNKNOWN_PRAGMA           => true,
	);
		...
		if (isset($options['pragmas'])) {
			foreach ($options['pragmas'] as $pragma_name => $pragma_value) {
				if (!in_array($pragma_name, $this->_pragmasImplemented, true)) {
					throw new KiTT_Mustache_Exception('Unknown pragma: ' . $pragma_name, KiTT_Mustache_Exception::UNKNOWN_PRAGMA);
				}
			}
	...
	protected function _findSection($template) {
			...
			switch ($type) {
				...
				case '/':
					if (empty($section_stack) || ($tag_name !== array_pop($section_stack))) {
						if ($this->_throwsException(KiTT_Mustache_Exception::UNEXPECTED_CLOSE_SECTION)) {
							throw new KiTT_Mustache_Exception('Unexpected close section: ' . $tag_name, KiTT_Mustache_Exception::UNEXPECTED_CLOSE_SECTION);
						}
					}
		...
		if (!empty($section_stack)) {
			if ($this->_throwsException(KiTT_Mustache_Exception::UNCLOSED_SECTION)) {
				throw new KiTT_Mustache_Exception('Unclosed section: ' . $section_stack[0], KiTT_Mustache_Exception::UNCLOSED_SECTION);
			}
		}
	}
	 ...
	 * @param mixed $matches
	 * @return void
	 * @throws KiTT_Mustache_Exception unknown pragma
	 */
	protected function _renderPragma($matches) {
		...
		if (!in_array($pragma_name, $this->_pragmasImplemented)) {
			if ($this->_throwsException(KiTT_Mustache_Exception::UNKNOWN_PRAGMA)) {
				throw new KiTT_Mustache_Exception('Unknown pragma: ' . $pragma_name, KiTT_Mustache_Exception::UNKNOWN_PRAGMA);
			} else {
				return '';
			}
	 ...
	 *
	 * Expects exceptions to be KiTT_Mustache_Exception error codes (i.e. class constants).
	 *
	 * @access protected
	 ...
	 * @throws KiTT_Mustache_Exception Unmatched section tag encountered.
	 * @return string
	 */
	 ...
	 * @access protected
	 * @param string $tag_name
	 * @throws KiTT_Mustache_Exception Unknown variable name.
	 * @return string
	 */
	...
	protected function _findVariableInContext($tag_name, $context) {
		...
		if ($this->_throwsException(KiTT_Mustache_Exception::UNKNOWN_VARIABLE)) {
			throw new KiTT_Mustache_Exception("Unknown variable: " . $tag_name, KiTT_Mustache_Exception::UNKNOWN_VARIABLE);
		} else {
			return '';
		}
	}
	 ...
	 * @access protected
	 * @param string $tag_name
	 * @throws KiTT_Mustache_Exception Unknown partial name.
	 * @return string
	 */
	protected function _getPartial($tag_name) {
		...
		if ($this->_throwsException(KiTT_Mustache_Exception::UNKNOWN_PARTIAL)) {
			throw new KiTT_Mustache_Exception('Unknown partial: ' . $tag_name, KiTT_Mustache_Exception::UNKNOWN_PARTIAL);
		} else {
			return '';
		}
	}
