...
class Mage_Checkout_Block_Cart_Item_Renderer extends Mage_Core_Block_Template
{
    ...
    {
        $item = Mage::getModel('sales/quote_item')->load($this->getItem()->getId());
        ...
        $personalized = $BuyRequest->getData();

        if($personalized)
        {
           if(isset($personalized['prb_customized_image']) and !is_null($personalized['prb_customized_image']))
           {
                $params = array(
                    '_query' => array(
                    'product'=> $this->getItem()->getProduct()->getId(),
                    'itemId' => $this->getItem()->getId(),
                    'qty'    => $personalized['qty'] ? $personalized['qty'] : 1,
                    'edit'   => 1,
                    )
                );
                return $this->getUrl('personalize',$params);
           } 
        }
