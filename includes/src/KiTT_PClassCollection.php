...
class KiTT_PClassCollection
{
    ...
    public function update($sum, $page, $types)
    {
        ...
        foreach ($this->_kapi->getPClasses() as $pclass) {
            ...
            $totalCost = $this->_formatter->formatPrice(
            ...
            if ($monthlyCost > 0) {
                $formattedMonthlyCost = $this->_formatter->formatPrice(
                    $monthlyCost, $locale
                );
            }
    ...
    public function defaultPClass()
    {
        if ($this->_default instanceof KlarnaPClass) {
            ...
            $pclass = array(
                'id' => $id,
                'price' => $info['formattedMonthlyCost'],
    ...
    public function table()
    {
        ...
        foreach ($this->pclasses as $pid => $info) {
            ...
            $out[] = array(
                'id' => $pid,
                'description' => $info['pclass']->getDescription(),
                'cost' => $info['formattedMonthlyCost'],
                'default' => $default,
                'type' => ($info['pclass']->getType() === KlarnaPClass::ACCOUNT
                    ? "account" : "campaign")
            );
        }
        return $out;
